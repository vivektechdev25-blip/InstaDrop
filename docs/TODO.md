...
- [x] QA carousel extraction against real multi-slide posts — sourced 9 real carousel URLs via web search, all 9 confirmed live with correct slide counts and distinct full-resolution files after redesigning the extraction mechanism (the original never actually worked - returned 1 slide for a confirmed real carousel) and fixing a date-format mismatch bug found along the way. See [KNOWN_ISSUES.md](./KNOWN_ISSUES.md#carousel-support-fixed-and-verified-2026-08-04).
- [ ] **BLOCKED, needs user input:** QA private-account detection against a real private post. Genuinely can't be sourced in this environment - private posts aren't publicly searchable by definition, and creating a throwaway Instagram account needs phone/email verification unavailable here. Need either a real private post URL from an account the user owns/controls, or explicit sign-off to leave this unverified. See [KNOWN_ISSUES.md](./KNOWN_ISSUES.md#private-account-detection-genuinely-blocked-not-just-didnt-get-to-it-2026-08-04).
- [x] Give `packages/types` a real build step before the API Dockerfile can actually run in production — `tsc` now emits to `dist/`, `main`/`types` point there, a `prepare` script builds it automatically on `pnpm install`. Confirmed live: `docker build` succeeds and the built container actually runs and serves real requests (Zod, `@reelsavehub/types` resolution, and Playwright/Chromium scraping a real Instagram URL all confirmed working inside the container) — took four rounds of real bugs found via `docker build`/`docker run`, see [KNOWN_ISSUES.md](./KNOWN_ISSUES.md).
...
- [x] Build `PreviewCard` (video player, carousel swiper, per-slide download) and wire the real download trigger — confirmed live in an actual browser (Playwright screenshot) for image, reel, and (since) real carousel posts
