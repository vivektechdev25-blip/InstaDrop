...
## Files downloaded from ReelSaveNow failed on WhatsApp share ("Download failed"): fixed, same root cause as above

**Reported:** a file downloaded from ReelSaveNow failed with a generic WhatsApp error ("Download failed. The download was unable to complete.") when shared to another device.
...
- **The new `[...url]` catch-all route throwing an unhandled exception:** checked `extractFromCatchAllPath()` and its callers — decoding is wrapped in try/catch, and the garbage-path test (`reelsavenow.com/some/random/thing`) was re-confirmed live to return a clean `200` with the existing friendly error UI, not a crash. Also, an exception here would only affect `apps/web`, not explain `apps/api` stopping within 90 seconds of it.
...
**Not something to "fix":** this is a local-dev-only artifact of running background processes on a laptop that sleeps, not a defect in ReelSaveNow's code. **Production monitoring note:** the deployed target (Railway, see [DEPLOYMENT.md](./DEPLOYMENT.md)) is a managed container platform, not a laptop subject to OS sleep — this specific mechanism doesn't apply there. Regardless, once deployed, uptime/process-restart monitoring should be in place so that any *real* production crash (unlike this one) surfaces immediately rather than silently, since a production outage is far more costly than a paused local session.

## Own-private-content authenticated extraction: unverified (2026-08-05)

**Decision:** documented as unverified rather than launch-blocking for this feature, same standard applied to private-account detection above. The endpoint fails closed on any ambiguity, so the worst case of an incorrect guess is an over-eager `SESSION_EXPIRED`/`ACCESS_DENIED` rejection — never a wrong-content leak.

**Update — session-expiry detection confirmed live:** sent a well-formed-but-fake `sessionCookie` (correct character-class/length shape, not a real value) against a real public reel URL through the actual running endpoint. Got back a genuine `401 SESSION_EXPIRED` — confirming Instagram really does redirect an invalid/unrecognized session to `/accounts/login/`, and that `privateContentService.ts` correctly detects that redirect and maps it to the right error code, end-to-end. This closes out the `/accounts/login/`-redirect part of the original unverified claim below.

**What's still actually unverified:** `findViewerIdentity()` and `findContentOwner()` in `apps/api/src/services/privateContentService.ts` — these only run *past* the login-redirect check, i.e. only with a real, currently-valid session, which remains unavailable in this environment. They deep-search Instagram's embedded relay JSON for plausible key names — `viewer`/`logged_in_user`/`current_user` for the authenticated session, `owner.id` for the requested content, extending the already-confirmed-live pattern that `owner.username`/`is_private` exist on that same GraphQL shape for public posts. Neither specific key has been confirmed against a real authenticated Instagram response, so neither the successful-ownership-match happy path nor a genuine `ACCESS_DENIED` mismatch case has been exercised live yet.

**Why this can't be resolved here, same structural reason as private-account detection:** testing requires a real Instagram session cookie from an account with at least one real Reel/post. This project has no way to create one — a throwaway account's signup flow requires phone/email verification, neither available in this environment, and would likely hit the same anonymous-bot detection that blocks scraping in the first place (see the anonymous-scraping investigation above).

**What's confirmed live instead, as a partial substitute:** request validation (both fields), the separate 3-req/10min rate limiter tripping independently of the public endpoint's own budget, that a validation failure never echoes the submitted cookie value, and — via a live regression fetch of a real public reel before/after — that extracting the shared DOM-parsing helpers into `mediaExtractionHelpers.ts` introduced zero behavior change to the already-working public flow. See [SECURITY.md](./SECURITY.md#post-mvp-private-account-support-own-content-only--implemented-2026-08-05) and [ARCHITECTURE.md](./ARCHITECTURE.md#own-private-content-flow-session-cookie-authenticated).

**Also unverified, stated as an assumption:** whether the CDN URL returned in a successful response needs the session cookie again for the actual `GET /api/v1/download` step. The code assumes not — Instagram's CDN URLs for media are typically signed, time-limited, cookie-independent links, matching how the public flow's `download` step already works with no cookie involved. Not live-tested against a real authenticated response for the same reason as above.

**What would resolve this:** a real Instagram session cookie from an account the user owns, with at least one real Reel/post to test against (and, ideally, one expired-cookie case to confirm the `SESSION_EXPIRED` path).
